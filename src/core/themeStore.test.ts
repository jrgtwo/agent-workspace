...
describe('ThemeStore', () => {
  it('defaults to the Terminal theme', () => {
    expect(new ThemeStore().getState()).toEqual({ theme: 'terminal' })
  ...
  it('ignores invalid theme ids', () => {
    ...
    expect(store.getState()).toEqual({ theme: 'terminal' })
  })
  ...
  it('hydrates valid saved state and falls back on garbage', () => {
    ...
    expect(b.getState()).toEqual({ theme: 'terminal' })
  })

  it('exposes the three selectable themes in order', () => {
    expect(THEMES.map((t) => t.id)).toEqual(['terminal', 'terminal-crt', 'midnight'])
  })
